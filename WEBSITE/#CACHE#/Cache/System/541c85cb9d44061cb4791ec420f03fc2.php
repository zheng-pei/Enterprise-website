      ...
      <div class="span12">
        <div class="box">
          <div class="box-title">
            ...
            <div class="fr">
              <form name="searchform" action="<?php echo U('Pics/index');?>" method="post" class="form-horizontal">
                ...
                <option value="0">网站版本</option>
                 ...
                 <option value="lastedittime" <?php if($_order == 'lastedittime'): ?>selected="selected"<?php endif; ?>>按时间</option>
                 </select>&nbsp;
              ...
              <input name="title" type="text" class="input-text" value="<?php echo ($title); ?>" placeholder="图片标题"/>
          ...
          <div class="box-content nozypadding" style="width:100%;">
            <div class="row-fluid">
              <div class="span8 control-group">
                <a class="btn" href="<?php echo U('Pics/add');?>"> <i class="icon-plus"></i>
                  添加图片
              ...
              <table id="listTable" class="table table-hover table-nomargin table-bordered usertable dataTable">
                ...
                <tbody>
                  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                      ...
                      <td><img src="<?php echo ($vo["pic"]); ?>" width="50px;" height="40px"/></td>                   
                      <td><?php echo (date("Y-m-d H:i:s",$vo["lastedittime"])); ?></td>
                    ...
                    <td>
                      <?php if($vo['status'] == 1): ?><a href="javascript: drop_confirm('确定禁用？','<?php echo U('Pics/changestatus',array('id'=> $vo['id'],'status'=>2,'p'=>$p));?>')" class="btnra" title="禁用">
                          <i class="icon-stop"></i>
                          禁用
                        </a>
                        <?php else: ?>
                        <a href="javascript: drop_confirm('确定启用？','<?php echo U('Pics/changestatus',array('id'=> $vo['id'],'status'=>1,'p'=>$p));?>')" class="btnra" title="启用">
                          <i class="icon-play"></i>
                          启用
                        </a><?php endif; ?>
                      <a href="<?php echo U('Pics/edit',array('id'=> $vo['id']));?>" class="btnra" title="编辑">
                        <i class="icon-edit"></i>
                        编辑
                      </a>
                      <a href="javascript: G.ui.tips.confirm('确定删除？','<?php echo U("Pics/foreverdelete",array('id'=> $vo['id']));?>')" class="btnra" title="删除">
