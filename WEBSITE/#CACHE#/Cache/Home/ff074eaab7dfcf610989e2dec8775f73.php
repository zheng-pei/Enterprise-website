...
<div class="zhuren mrt30 tuandui">
  <!--团队列表-->
  <div class="jcon" >
    <ul class="zr_list">
      <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
          <div class="zr_list_left">
            <a href="<?php echo U('News/news',array('pid'=>1,'cateid'=>$cateid,'versionid'=>$versionid,'id'=>$vo['id']));?>">
              <img src="<?php echo ($vo['pic']); ?>" />
            </a>
          </div>
          <div class="zr_list_right">
            <h3 class="fs20 fonth">
              <a href="<?php echo U('News/news',array('pid'=>$pid,'cateid'=>$cateid,'versionid'=>$versionid,'id'=>$vo['id']));?>"><font color="<?php echo ($vo["color"]); ?>" class="abc"><?php echo ($vo["title"]); ?></font></a>
            </h3>
            <?php if($vo['subtitle'] != ''): ?><h4 class="fs16 fonth ells"><?php echo ($vo["subtitle"]); ?></h4><?php endif; ?>
            <div class="line">
              <hr /> <b></b>
            </div>
            <?php echo (msubstr(strip_tags(htmlspecialchars_decode($vo["content"])),0,450)); ?>
          </div>
        </li><?php endforeach; endif; else: echo "" ;endif; ?>
    </ul>
