...
<!--研究项目列表-->
   <div class="program mrt30">
       <ul>
        <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
         <a href="<?php echo U('News/news',array('pid'=>$pid,'cateid'=>$cateid,'versionid'=>$versionid,'id'=>$vo['id']));?>">
         <i class="arr fonth"><p><?php echo ($vo['code']); ?></p><?php if($versionid == 1): ?>项目<?php else: ?>project<?php endif; ?></i>
           <div class="p_right">
           <h3 class="ells fonth"><font color="<?php echo ($vo['color']); ?>" class="fs18"><?php echo ($vo["title"]); ?></font></h3>
            <p class="color84"><?php echo (msubstr(strip_tags(htmlspecialchars_decode($vo["content"])),0,500)); ?></p>
            <b class="color_blue xq"><?php if($versionid == 1): ?>点击查看详情<?php else: ?>Click to view the details<?php endif; ?>>></b>
           </div>
           </a>
         </li><?php endforeach; endif; else: echo "" ;endif; ?>
       </ul>
