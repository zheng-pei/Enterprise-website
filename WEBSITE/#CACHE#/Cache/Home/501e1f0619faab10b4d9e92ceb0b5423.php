...
<div class="content">
 ...
 <!--科研成果论文列表-->
   <div class="ky_lunwen mrt30">
     <ul>
       <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
         <h3>
         <a href="<?php echo U('News/news',array('pid'=>$pid,'cateid'=>$cateid,'versionid'=>$versionid,'id'=>$vo['id']));?>" class="fs18" title="<?php echo ($vo["title"]); ?>"><font color="<?php echo ($vo['color']); ?>" class="abc"><?php echo (msubstr($vo["title"],0,60)); ?></font></a>
         </h3>
          <?php if($vo['subtitle'] != ''): ?><span class="color84 lb"><?php echo (msubstr($vo["subtitle"],0,40)); ?></span><?php endif; ?>
         <p class="color84 con"><?php echo (msubstr(strip_tags(htmlspecialchars_decode($vo["content"])),0,150)); ?></p>
         <a href="<?php echo U('News/news',array('pid'=>$pid,'cateid'=>$cateid,'id'=>$vo['id']));?>" class="color_blue xq"><?php if($versionid == 1): ?>点击展开阅读<?php else: ?>Click to view the details<?php endif; ?>>></a>
       </li><?php endforeach; endif; else: echo "" ;endif; ?>
     </ul>
