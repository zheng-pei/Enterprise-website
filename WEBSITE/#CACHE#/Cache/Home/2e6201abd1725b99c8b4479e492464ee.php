<?php if (!defined('THINK_PATH')) exit();?><!-- 通用的列表内容页 -->
...
<!-- 文章内容页 -->
<div class="jianjie mrt30">
       <!--实验室简介-->
       <div class="jcon">
       		<!-- 文章标题 -->
				<?php if($article["isstrong"] == 1): ?><div class="bt"><strong><font color="<?php echo ($article["color"]); ?>"><?php echo ($article["title"]); ?></font></strong></div>
				<?php else: ?>
				<div class="bt"><font color="<?php echo ($article["color"]); ?>"><?php echo ($article["title"]); ?></font></div><?php endif; ?>
				<!-- 显示子标题 -->
				<?php if($article["subtitle"] != ''): ?><p class="bt"><?php echo ($article["subtitle"]); ?></p><?php endif; ?>
				<center><p style="color:#666;padding-top:10px;padding-bottom:25px;">&nbsp;&nbsp;时间：<?php echo (date("Y-m-d",$article["createtime"])); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;来源: <?php echo ($article["source"]); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;浏览量:<?php echo ($article["hits"]); ?>次</p></center>
				         	
				         	<?php if($columnmodel == 1): ?><!-- 文字的内容 -->
								<?php echo (htmlspecialchars_decode($article["content"])); ?>
							<?php elseif($columnmodel == 2): ?>
						<!-- 图文的内容 -->
								<?php echo (htmlspecialchars_decode($article["content"])); ?>
							<?php elseif($columnmodel == 3): ?>
						<!-- 图片集的内容 -->
							<?php if(is_array($pics)): $i = 0; $__LIST__ = $pics;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul >
									<li class="so">
										<center>
										<img  height="200px;" src="<?php echo ($vo[0]); ?>" title="<?php echo ($vo[1]); ?>"/><br/>
										<!-- 图片标题： --><?php echo ($vo[1]); ?><br/><br/>
										<!-- 图片描述： --><?php echo ($vo[2]); ?>
										</center>
									</li>
     ...
     <div class="news_list">
         <ul>
         <!-- 先判断是否存在上一篇或下一篇 -->
         	<?php if(($prev != 0)): ?><li>上一篇：<a href="<?php echo U('News/news',array('pid'=>$pid,'cateid'=>$cateid,'versionid'=>$versionid,'id'=>$prev));?>"><font color="<?php echo ($prev_color); ?>"><?php echo ($prev_title); ?></font></a></li>
           	<?php else: ?>
           	<li>上一篇： 没有了</li><?php endif; ?>
			<?php if(($next != 0)): ?><li>下一篇：<a href="<?php echo U('News/news',array('pid'=>$pid,'cateid'=>$cateid,'versionid'=>$versionid,'id'=>$next));?>"><font color="<?php echo ($next_color); ?>"><?php echo ($next_title); ?></font></a>
          		 </li>
          		<?php else: ?>
