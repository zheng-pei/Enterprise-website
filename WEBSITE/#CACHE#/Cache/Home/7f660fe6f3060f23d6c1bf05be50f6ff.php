...
<!-- 新闻列表 -->
   <div class="jiaoliu mrt30">
         <ul class="jl_list">
          <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
            <div class="jl_list_left fr"><img src="<?php echo ($vo["pic"]); ?>"/></div>
            <div class="jl_list_right">
               <h3>
                  <a href="<?php echo U('News/news',array('pid'=>$pid,'cateid'=>$cateid,'versionid'=>$versionid,'id'=>$vo['id']));?>" class="fs18 ells" title="<?php echo ($vo["title"]); ?>"><font color="<?php echo ($vo["color"]); ?>" class="abc"><?php echo ($vo["title"]); ?></font></a></span><b class="fr"><?php echo (date("Y/m/d",$vo["createtime"])); ?></b>
               </h3>
               <?php echo (msubstr(strip_tags(htmlspecialchars_decode($vo["content"])),0,360)); ?>
               <a class="color_blue xq" href="<?php echo U('News/news',array('pid'=>$pid,'cateid'=>$cateid,'versionid'=>$versionid,'id'=>$vo['id']));?>"><?php if($versionid == 1): ?>点击展开阅读<?php else: ?>Click to view the details<?php endif; ?>>></a>
             </div>
            </li><?php endforeach; endif; else: echo "" ;endif; ?>
         </ul>
