       ...
       <!--科研奖励-->
         <ul class="jl_list">
           <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
            <div class="jl_list_left fl"> <a href="<?php echo U('News/news',array('pid'=>$pid,'cateid'=>$cateid,'versionid'=>$versionid,'id'=>$vo['id']));?>"><img src="<?php echo ($vo["pic"]); ?>" /></a></div>
             <div class="jl_list_right">
               <h3>
                 <a href="<?php echo U('News/news',array('pid'=>$pid,'cateid'=>$cateid,'versionid'=>$versionid,'id'=>$vo['id']));?>" class="fs18 ells" title="<?php echo ($vo["title"]); ?>"><font color="<?php echo ($vo["color"]); ?>" class="abc"><?php echo ($vo["title"]); ?></font></a>
               </h3>
               <?php if($vo['subtitle'] != ''): ?><h4><?php echo ($vo["subtitle"]); ?></h4><?php endif; ?>
               <hr />
               <?php echo (msubstr(strip_tags(htmlspecialchars_decode($vo["content"])),0,300)); ?>
             </div>
            </li><?php endforeach; endif; else: echo "" ;endif; ?>
         </ul>
