...
<!-- 代表成果列表的模板 -->
   <div class="ky_program mrt30">
     <ul class="cg_list clearfix">
     <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
         <div class="cg_left_left"><i class="quare"></i></div>
         <div class="cg_list_right">
           <h3>
               <a href="<?php echo U('News/news',array('pid'=>$pid,'cateid'=>$cateid,'versionid'=>$versionid,'id'=>$vo['id']));?>" class="fs18" title="<?php echo ($vo["title"]); ?>" title="<?php echo ($vo["title"]); ?>"><font color="<?php echo ($vo['color']); ?>" class="abc"><?php echo ($vo["title"]); ?></font></a>
           </h3>
            <?php if($vo['subtitle'] != ''): ?><h4 class="subtitle"><?php echo (msubstr($vo["subtitle"],0,90)); ?></h4><?php endif; ?>
           <hr />
           <p class="con"><?php echo (msubstr(strip_tags(htmlspecialchars_decode($vo["content"])),0,160)); ?></p>
